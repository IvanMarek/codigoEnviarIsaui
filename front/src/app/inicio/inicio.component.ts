...
export class InicioComponent implements OnInit {
  ...
  constructor(private loginService: LoginService, private seguimientoService: SeguimientoService) {}
  ...
  cargarUsuarios(): void {
    this.loginService.getUsuarios().subscribe(
      (usuarios: Usuario[]) => { // Especifica el tipo aquí
  ...
  actualizarEstadoUsuarios(): void {
    ...
    this.usuariosRecomendados.forEach(usuario => {
      usuario.seguido = this.seguimientos.some(seg => seg.seguidoId === usuario.usuarioId);
    });
  }
  ...
  crearPublicacion() {
    const nuevaPublicacion = {
      ...
      autor: 'Nombre del Usuario' // Cambia esto por el nombre del usuario logueado
    };
